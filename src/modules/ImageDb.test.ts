...
beforeAll(async () => {
	await db.init()
	await db.wipe()
	for (let file of fs.readdirSync(imageDir)) if (file.startsWith('test-')) fs.unlinkSync(imageDir + file)
})
...
test('downloads a file', async () => {
	const downloadedPath = './downloads/test-id1.png'
	if (fs.existsSync(downloadedPath)) fs.unlinkSync(downloadedPath)

	await db.downloadImage({ id: 'test-id1', url: 'https://dummyimage.com/600x400/000/fff.png', date_queued: Date.now() })
	expect(fs.existsSync(downloadedPath)).toBe(true)
})

test('writes to database after downloading', async () => {
	await db.downloadImage({ id: 'test-id1', url: 'https://dummyimage.com/600x400/000/fff.png', date_queued: Date.now() })
	let item = await db.getbyId('test-id1')
